...
__Running title:__ Sex signaling effects on firefly antenna 

# Abstract

Fireflies (family Lampyridae) are known for their bioluminescent organs which flash or glow to mediate sexual reproduction in many species. Other species of fireflies have secondarily reverted to using olfactory signals (pheromones) to attract mates. In this study, we investigate the primary olfactory organ of fireflies (antenna) to determine whether the reversal to chemical signaling has affected the morphometrics of antennae and its olfactory sensilla. We hypothesize that species using chemical signaling and male animals would have greater densities and counts of olfactory sensilla responsible for capturing pheromones, as well as longer and wider olfactory sensilla, compared to species using visual signals. We find that neither signaling strategy, sex, nor their combination have significant relationships with any morphometric measurement examined. Olfactory sensilla arrays are likely used for a variety of tasks not linked to sexual signaling strategies. The form and arrangement of olfactory sensilla could represent the smallest, most compact, and inexpensive way to create and maintain these arrays. Consistent with previous work, no differences between male and female animals suggest that females possess the same sensilla that react to sex pheromones but either they are not physiologically active or these signals are ignored. 

## Keywords

Lampyridae, odor capture, pheromone, sexual selection, antenna, sensilla
...
Individual samples of species were acquired from the entomology collection of the Los Angeles County Natural History Museum, representing 32 individual animals and 26 species (see supplementary material for complete list). Each specimen was destructively sampled by the removal of an entire antenna. Only specimens with intact antennae were sampled, although during sampling and transport to Chapman University, some segments were lost. Each specimen was photographed to recover body length and sex. Two specimens lacked abdominal segments and were unable to be sexed, and these are marked as "NA" or "not avaiable." 
...
Measurements were not normalized to body length because each measure was determined to be independent of body length through linear regression. Each statistical analysis was repeated taking body length into account and there were no changes in significance for any measure. Additional statistical values can be found in supplementary material. 


## Data Availability

All measurements and code necessary to reproduce the phylogenetic tree, calculations, figures, and statistical analyses are available on Github: (redacted for review). 


# Results

## Signaling strategy does not affect sensillum morphometrics or arrangement
...
Table \@ref(tab:signal-ptable) presents $p$-values and sample sizes of phylogenetically corrected ANOVAs for various comparisons of sensilla against signaling strategy. Full test statistics can be found in supplementary material. Figure \@ref(fig:signal-plots) presents results for species means with ranges divided by signaling strategy. Both olfactory and mechanosensory sensilla share similar widths. Species mean olfactory sensillum widths are 
...
$`r signif(mean_width_sex$means[mean_width_sex$sensillum_type == "olfactory" & mean_width_sex$sex == "female"][1], digits = 3)` \pm `r signif(mean_width_sex$sds[mean_width_sex$sensillum_type == "olfactory" & mean_width_sex$sex == "male"][1], digits = 1)` ~\mu$m for females and 
...
p_pic1 <- ggdraw() + draw_image("./doc/figures/figure1/Lucidota_sp.JPG")
p_pic2 <- ggdraw() + draw_image("./doc/figures/figure1/Photinus_consanguineus.JPG")
...
BBBBBD
"
free(ggdraw() + draw_image("./doc/figures/figure1/SEM_fig.png") + plot_layout(tag_level = "new") )+
  p_tree + p_pic1 + p_pic2 +
  plot_layout(design = design) +
  plot_annotation(tag_levels = list(c("c", "d" ,"e"))) & theme(plot.tag = element_text(color = "black", 
                                                                    face = "plain"))
if(save_figs) {
  dir.create("./doc/figures/figure1/", showWarnings = F)
  ggsave("./doc/figures/figure1/bug_tree.pdf", last_plot(), height = 9.0, width = 7.5)
}
```
...
if(save_figs){
  dir.create("./doc/figures/figure2/", showWarnings = F)
  ggsave("./doc/figures/figure2/signal-plots.pdf", last_plot(), 
...
if(save_figs){
  ...
  ggsave("./doc/figures/figure3/count-plots.pdf", last_plot(), height = 4.25, width = 6.5)
}
```
...
if(save_figs){
  ...
  ggsave("./doc/figures/figure4/sex-plots.pdf", last_plot(), height = 5.5, width = 6.5)
}
```
...
if(save_figs){
  ...
  ggsave("./doc/figures/figure5/breakdown-plots.pdf", last_plot(), height = 5.5, width = 6.5)
}
```
